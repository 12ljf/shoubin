...
"""
RTK + IMU406 地下管道定位（修正版）

每一帧 IMU 数据会打印：
1. [本机时间] + [GNSS时间]
2. [RTK 原始] 当前洞口 RTK 经纬度 / 解状态 / 原始 GGA
3. [IMU 积分] 前进距离 + ENU 东北平面位移
4. [融合坐标] RTK 锚点 + ENU 位移换算后的经纬度
"""
...
# RTK 当前值（原始 + 锚点）
rtk_lat = None         # 当前 RTK 纬度
rtk_lon = None         # 当前 RTK 经度
...
def imu_loop():
    ...
    global forward_vel, forward_dist, enu_east, enu_north
    ...
    while True:
        ...
        while True:
            ...
            if fused_lat is not None and fused_lon is not None:
                print(
                    f"[融合坐标] 经度={fused_lon:.8f}, 纬度={fused_lat:.8f} "
                    f"(相对 RTK 锚点偏移)"
                )
            else:
